...
const PortfolioContent = () => {
  ...
  if (currentView === 'privacy') return html`<${PrivacyPolicy} onBack=${() => setCurrentView('home')} />`;
  if (currentView === 'terms') return html`<${TermsOfService} onBack=${() => setCurrentView('home')} />`;
        ...
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <div className="md:hidden">
            <button onClick=${() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-white p-2">
              ${isMobileMenuOpen ? html`<${X} className="w-6 h-6" />` : html`<${Menu} className="w-6 h-6" />`}
            </button>
          </div>
        </div>
