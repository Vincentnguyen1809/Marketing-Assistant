...
export const Journey = () => {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'start': return html`<${Flag} className="w-6 h-6 text-blue-500" />`;
      case 'fail': return html`<${AlertTriangle} className="w-6 h-6 text-red-500" />`;
      case 'success': return html`<${CheckCircle2} className="w-6 h-6 text-green-500" />`;
      case 'scale': return html`<${Award} className="w-6 h-6" style=${{color: theme.primaryColor}} />`;
      default: return html`<${TrendingUp} className="w-6 h-6 text-slate-500" />`;
    }
  };
    ...
    <section className="py-32 bg-slate-50">
      <div className="container mx-auto px-6">
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          ${milestones.map((milestone, index) => html`
