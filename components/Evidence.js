...
export const Evidence = () => {
  ...
  const getPointCoordinates = (index) => {
    ...
    const padding = 20;
    const x = (index / maxYearIndex) * (width - padding * 2) + padding;
    const y = height - ((chartData[index].revenue / maxRevenue) * (height - padding * 2)) - padding;
    return { x, y };
  };
    ...
    <section className="py-32 bg-slate-950 text-white overflow-hidden relative">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-20">
          <div className="inline-flex items-center gap-2 mb-4 font-bold tracking-widest uppercase text-xs md:text-sm" style=${{ color: theme.primaryColor }}>
          ...
          <p className="text-slate-400 max-w-2xl mx-auto text-lg leading-relaxed font-normal">
            In an industry full of promises, we deliver verifiable results.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-24">
          ${metrics.map((metric, index) => html`
            <div key=${index} className="p-8 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-amber-500/50 transition-all group backdrop-blur-sm">
              <div className="text-4xl md:text-5xl font-bold text-white mb-2 transition-colors" style=${{ color: theme.primaryColor }}>
                ${metric.value}<span className="text-2xl text-slate-500">${metric.suffix}</span>
              </div>
        ...
        <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800 shadow-2xl mb-24" ref=${chartRef}>
           <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
              <div>
                ...
                <p className="text-slate-500 text-sm">Client aggregate data</p>
              </div>
                ...
                ${chartData.map((d, i) => {
                  ...
                  return html`
                    <g key=${i} className=${`transition-opacity duration-500 delay-[${1000 + i * 100}ms] ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
                      <circle cx=${x} cy=${y} r="6" fill="#1e293b" stroke=${theme.primaryColor} strokeWidth="3" className="hover:r-8 transition-all cursor-pointer" />
                      <text x=${x} y=${395} textAnchor="middle" fill="#94a3b8" fontSize="12" fontWeight="bold" dy="15">
                        ${d.year}
                      </text>
                      <text x=${x} y=${y - 15} textAnchor="middle" fill="white" fontSize="12" fontWeight="bold" className="opacity-0 md:opacity-100">
        ...
        <div className="mb-32">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
            <div className="grid grid-cols-12 gap-4 p-4 bg-slate-800/50 text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-800">
              ...
              <div className="col-span-2 md:col-span-2 text-right">Results</div>
              <div className="col-span-2 md:col-span-2 text-right">Cost Per Result</div>
            </div>

            ${liveCampaigns.map((camp, idx) => html`
              <div key=${idx} className="grid grid-cols-12 gap-4 p-5 items-center border-b border-slate-800 hover:bg-slate-800/30 transition-colors">
                <div className="col-span-5 md:col-span-4 flex items-center gap-3">
                   ${camp.platform.includes('Google') ? html`
                ...
                <div className="col-span-2 md:col-span-2 text-right">
                  <div className="font-bold" style=${{ color: theme.primaryColor }}>${camp.cpl}</div>
        ...
        <div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ${testimonials.map((testimonial, idx) => html`
              <div key=${idx} className="bg-slate-900 border border-slate-800 rounded-2xl p-8 relative flex flex-col hover:border-amber-500/30 transition-colors duration-300">
                ...
                <div className="flex items-center gap-4 mb-6">
                  <img src=${testimonial.image} alt=${testimonial.name} className="w-16 h-16 rounded-full object-cover border-2" style=${{ borderColor: theme.primaryColor }} />
                  <div>
                ...
                <div className="flex-1">
                  <h5 className="font-bold mb-3 text-lg" style=${{ color: theme.primaryColor }}>"${testimonial.quote}"</h5>
                  <p className="text-slate-300 text-sm leading-relaxed italic opacity-90">
                    ${testimonial.story}
                  </p>
                </div>

                <div className="mt-6 flex gap-1" style=${{ color: theme.primaryColor }}>
                  ${[...Array(5)].map((_, i) => html`<${Star} key=${i} className="w-4 h-4 fill-current" />`)}
                </div>
              </div>
            `)}
          </div>
